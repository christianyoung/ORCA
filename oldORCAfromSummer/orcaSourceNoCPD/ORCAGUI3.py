...
def create_widgets_in_select_gangfac_frame(path,gangFac):
    def verifyinput(path,PrintableNames,selection):
        if selection in PrintableNames:
            ...
            tkMessageBox.showerror("Error","The gangname entered is not recognized.")
    ...
    tacpdfButton=Button(textFrame,text = "Tactical PDF",command=lambda:tacPDF(verifyinput(path,PrintableNames,selection.get())),width = 20)
