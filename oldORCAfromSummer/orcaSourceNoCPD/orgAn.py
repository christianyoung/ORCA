...
def graphSubOrgs(subOrgToMem, memsToSub,overallGraphFileLocation, overallGraphFile,ndpth,irToStr):
									...
									for neighCom in memsToSub[neigh]:
										...
										if not (neighCom in memsToSub[node.split('(')[0]]):
												if not(neighCom in edgeDict[sub]):
														...
														edgeConnDict[sub][neighCom]=[]
												edgeDict[sub][neighCom]+=1
												theStr = neigh.split('(')[0].strip('\n')
												if not (theStr in edgeConnDict[sub][neighCom]):
													edgeConnDict[sub][neighCom].append(theStr)
						
						#for inGroup in memsToSub[node]:
					#		if inGroup in neighComsOfNode[node]:
					#			neighComsOfNode[node].remove(inGroup)
										
		for sub in edgeConnDict:
			for neighCom in edgeConnDict[sub]:
				if sub in edgeConnDict[neighCom]:
					for mem in edgeConnDict[neighCom][sub]:
						if not (mem in edgeConnDict[sub][neighCom]):
							edgeConnDict[sub][neighCom].append(mem)
		ecd = {}
		for sub in edgeDict:
				ecd[sub]={}
				for neighSub in edgeDict[sub]:
						ecd[sub][neighSub]=","
						for mem in edgeConnDict[sub][neighSub]:
							#if not mem in ecd[sub][neighSub]:
							ecd[sub][neighSub]=ecd[sub][neighSub]+mem+","
						if not( (((sub,neighSub) in ResG.edges() ) or ((neighSub,sub) in ResG.edges() ) )):
								#ResG.add_edge(sub,neighSub, weight = edgeDict[sub][neighSub])
								ResG.add_edge(sub,neighSub, weight = len(edgeConnDict[sub][neighSub]))
						
		for sub in ecd:
			for neighSub in ecd[sub]:
				if not(sub in ecd[neighSub]):
					ecd[neighSub][sub]=ecd[sub][neighSub]
		cnNodeTupeList = []
		...
		return ResG,neighComsOfNode,ecd
		
def subGroupRelGraph(groupName,ResG,orgToSubOrg,thePath,subOrgToOrg,nextStep,ncon,orgToALlMem,memsToSub,ndpth,irToStr,ecd):
		...
		for sub in orgToSubOrg[groupName]:
				...
				for neigh in ResG.neighbors(sub):
						...
						if wt >minLinks-1:
							 ...
							 #print ecd[sub][neigh]
							 conCount += 1	
							 bullets.write(",,"+neigh+"  ("+str(wt)+" connections): "+ecd[sub][neigh]+"\n")
				...
				if conCount == 0:
					bullets.write(",,None (relatively isolated - all other groups are connected with less than "+str(minLinks)+" links)\n")
				bullets.write("\n")
		bullets.close()
...
def GtoND(G,fulloutpath,ndpth,irToStr,initStr):
	fulloutpath = fulloutpath.split('.')[0]+".vna"
	beginPart=fulloutpath.split('.')[0]
...
def analyzeAllOrgs(thePath, initFolder,oldEdgeList,inFile1,inFile2,delim):
		...
		ResG,ncon,ecd=graphSubOrgs(subOrgToMem, memsToSub,os.path.join(thePath,initFolder), overallGraphFile,ndpth,irToStr)
		subOrgToOrg={}
		for org in orgToSubOrg:
				for sub in orgToSubOrg[org]:
						subOrgToOrg[sub]=org
		for groupName in orgToSubOrg:
				subGroupRelGraph(groupName,ResG,orgToSubOrg,os.path.join(thePath,initFolder),subOrgToOrg,False,ncon,orgToAllMem,memsToSub,ndpth,irToStr,ecd)
