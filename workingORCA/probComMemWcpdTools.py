...
def runConversion(thePath,initFolder,gangOrFac,delim):
        ...
        convertInfoAndRunNewLearner(thePath,initFolder,oldEdgeList,inputFile1,inputFile2,gangInputFile,int(gangOrFac),delim)    
...
#arrest data input format based on cpd
def combine3(inFolder,outfullpathfilename,dateLimit):
        global IRSet
        outf = open(outfullpathfilename,'w')
        flag = True
        for filename in os.listdir(inFolder):
                ...
                with open(fullpath,'r') as fh:
                        ...
                        for line in fh.readlines():
                                ...
                                if (str(lsp[0]).strip() not in IRSet) and (datetime.strptime(dateString, "%d%m%Y").date() >= dateLimit):
                                        outf.write(line)
        outf.close()

def combine4(inFolder,outfullpathfilename):
        outf = open(outfullpathfilename,'w')
        flag = True
        for filename in os.listdir(inFolder):
                if filename:
                        ...
                        with open(fullpath,'r') as fh:
                                ...
                                for line in fh.readlines():
                                        if line.strip() != "":
                                                outf.write(str(line.strip())+'\n')
        outf.close()

#arrest data input format based on cpd
def combineOrg(inFolder,outfullpathfilename,dateLimit):
        global IRSet
        outf = open(outfullpathfilename,'w')
        flag = True
        for filename in os.listdir(inFolder):
                ...
                with open(fullpath,'r') as fh:
                        ...
                        for line in fh.readlines():
                                ...
                                if len(dateStringList[2]) == 2:
                                        dateStringList[2] = '20'+dateStringList[2]
                                dateString = str(dateStringList[0])+str(monthToInt(dateStringList[1]))+str(dateStringList[2])
                                if (first == " " and second == " ") or (str(sp[0]).strip() in IRSet) or (datetime.strptime(dateString, "%d%m%Y").date() < dateLimit):
                                        toAdd=False
                                if toAdd:
...
#non arrest data input format based on cpd
def procCont(nonArPth,outFileList,outFileOrg,dateLimit):
    global IRSet
    ...
    for fileName in os.listdir(nonArPth):
        fullPathFileName = os.path.join(nonArPth,fileName)
        with open(fullPathFileName, 'r') as fh:
            for line in fh.readlines()[1:]:
                ...
                orgTupe = GANG.strip("\n"),FACTION_NAME.strip("\n"),idNum,cid
                dateStringList = lineList[12].split('-')
                dateStringList[1] = str(monthToInt(dateStringList[1]))
                dateStringList[2] = dateStringList[2].split(' ')[0].strip()
                if len(dateStringList[0]) == 1:
                        dateStringList[0] = '0'+dateStringList[0]
                if len(dateStringList[1]) == 1:
                        dateStringList[1] = '0'+dateStringList[1]
                if len(dateStringList[2]) == 2:
                        dateStringList[2] = '20'+dateStringList[2]
                dateString = str(dateStringList[0])+str(dateStringList[1])+str(dateStringList[2])
                if (datetime.strptime(dateString, "%d%m%Y").date() >= dateLimit) and (str(idNum).strip() not in IRSet):
...
def convRelList(fullPathRelFile):
    of = open(removeFileExtensions(fullPathRelFile)+"-OUT.dsv", 'w')
...
def initDirs(thePath,initFolder, arRecLoc,arEdgeLoc,nonArRecLoc,nonArEdgeLoc,remNodesLoc,dateLimit):
        global IRSet
...
#                        shutil.copy2(fromFile,toFile)
        remDir = os.path.join(pth2,"remNodeList.dsv")
        combine4(remNodesLoc,remDir)
        rem = open(remDir,'r')
        for line in rem:
                IRSet.add(str(line).strip())
        combine3(arRecLoc,os.path.join(pth2,"AR-RECLIST.dsv"),dateLimit)
        combine2(arEdgeLoc,os.path.join(pth2,"AR-EDGELIST.dsv"))
        combineOrg(arRecLoc,os.path.join(pth2,"AR-ORGLIST.dsv"),dateLimit)
        combine2(nonArEdgeLoc,os.path.join(pth2,"CC-EDGELIST.dsv"))
        procCont(nonArRecLoc,os.path.join(pth2,"CC-RECLIST.dsv"),os.path.join(pth2,"CC-ORGLIST.dsv"),dateLimit)
...
def getGangAndFacDicts(pathName,fileName,G,gangOrFac,delim):
    ...
    with open(os.path.join(pathName,fileName), 'r') as fh:
        for line in fh.readlines()[1:]:
            ...
            if int(gangOrFac) == 1:
                                fac = "FAC-"+ridSpaces(sp[int(gangOrFac)])
            else:
                                fac = "GANG-"+ridSpaces(sp[int(gangOrFac)])
            ...
            if len(sp[0])>1 and sp[0].find("UNKNOWN")==-1 and sp[0].find("UNIDENTIFIED")==-1:
                ...
                if not (gang in byGang):
                    byGang[gang]=set()
                byGang[gang].add(ir)
            if len(sp[int(gangOrFac)])>1 and sp[int(gangOrFac)].find("UNKNOWN")==-1 and sp[int(gangOrFac)].find("UNIDENTIFIED")==-1:
                facDic[ir]=fac
...
def crGphAndFacFilesCPD(thePath,oldEdgeList,newEdgeList,interpFile,comDir,inputFile1,inputFile2,gangInputFile,gangOrFac,delim):
    ...
    gangDic, facDic, byGang, byFac = getGangAndFacDicts(thePath,gangInputFile,G,int(gangOrFac),delim)
...
def convertInfoAndRunNewLearner(thePath,initFolder,oldEdgeList,inputFile1,inputFile2,gangInputFile,gangOrFac,delim):
        ...
        G = crGphAndFacFilesCPD(thePath,oldEdgeList,newEdgeList,interpFile,comDir,inputFile1,inputFile2,gangInputFile,int(gangOrFac),delim)
